...
*/
  ...
  class ht_product_keywords_insert {
    var $code = 'ht_product_keywords_insert';
    ...
    function ht_product_keywords_insert() {
      $this->title = MODULE_HEADER_TAGS_PRODUCT_KEYWORDS_INSERT_NEW_TITLE;
      $this->description = MODULE_HEADER_TAGS_PRODUCT_KEYWORDS_INSERT_NEW_DESCRIPTION;

      if ( defined('MODULE_HEADER_TAGS_PRODUCT_KEYWORDS_INSERT_STATUS') ) {
        $this->sort_order = MODULE_HEADER_TAGS_PRODUCT_KEYWORDS_INSERT_SORT_ORDER;
        $this->enabled = (MODULE_HEADER_TAGS_PRODUCT_KEYWORDS_INSERT_STATUS == 'True');
      }
    }
    ...
    function execute() {
      ...
      if( basename( $PHP_SELF ) == FILENAME_PRODUCT_INFO && isset( $_GET['products_id'] ) && $product_check['total'] > 0 && (!isset($_GET['manufacturers_id']) || $_GET['manufacturers_id'] == 0 ) ) {
        ...
        if( $products_id > 0 ) {
          ...
          $oscTemplate->addBlock( $head_keywords, $this->group );  // Save the new title string
        } // if( $products_id
    ...
    function check() {
      return defined('MODULE_HEADER_TAGS_PRODUCT_KEYWORDS_INSERT_STATUS');
    }
    ...
    function install() {
      ...
      if( $head_keywords == false ) {
        ...
        tep_db_query( "alter table " . TABLE_PRODUCTS_DESCRIPTION . " add column head_keywords varchar(255) NOT NULL default '' after " . $insert_after );
      }

      tep_db_query( "insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Enable Product Keywords Module', 'MODULE_HEADER_TAGS_PRODUCT_KEYWORDS_INSERT_STATUS', 'True', 'Do you want to add custom keywords to the product meta keyword tag?', '6', '1', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())" );
      tep_db_query( "insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_HEADER_TAGS_PRODUCT_KEYWORDS_INSERT_SORT_ORDER', '200', 'Sort order of keywords. Meta tags are duplicated, so add only one per page.', '6', '2', now())" );
    }
    ...
    function keys() {
      ...
      $keys_array[] = 'MODULE_HEADER_TAGS_PRODUCT_KEYWORDS_INSERT_STATUS';
      $keys_array[] = 'MODULE_HEADER_TAGS_PRODUCT_KEYWORDS_INSERT_SORT_ORDER';
