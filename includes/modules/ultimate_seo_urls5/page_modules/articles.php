<?php
  ...
  * @Id $Id:: articles.php 199 2011-01-26 15:27:34Z Rob                 $:  Full Details   
  */
  ...
  * @package USU5_PRO
  */
  class Articles_Page_Module extends aPage_Modules {
    ...
    */
    protected $cache_name_builder = array( 'tPath' => 1, 'authors_id' => 1 ); // xxx = _GET key ( e.g. cPath ), you may want to add "page" if there are paging functions
    /**
    * Class constructor
    * @access private
    */
    ...
    */
    public function buildLink( $page, $parameters, $add_session_id, $connection ) {
      $extract = array( 'tPath', 'authors_id' );
      ...
      */
      ...
      switch ( true ) {
        case $this->key == 'tPath': // xxx = _GET key ( e.g. cPath )
          ...
          $this->setQuery( array( TABLE_TOPICS_DESCRIPTION, $this->stripPathToLastNumber( $this->keys_index[$this->key] ), Usu_Main::i()->getVar( 'languages_id' ) ) );
          break;
        case $this->key == 'authors_id': // xxx = _GET key ( e.g. cPath )
          // This array contains replacements for the to_replace array ( see the $dependencies array )
          $this->setQuery( array( TABLE_AUTHORS, $this->stripPathToLastNumber( $this->keys_index[$this->key] ) ) );
          break;
        default:
